...
export const LeadPreview = ({ leads, isPurchased = false }: LeadPreviewProps) => {
  ...
  const formatLocation = (location: string, showFull: boolean) => {
    const parts = location.split(',').map(part => part.trim());
    if (parts.length < 2) return location;
    
    // For available leads, only show city and state
    if (!showFull) {
      return parts.slice(-2).join(', '); // Returns "City, State"
    }
    
    // For purchased leads, show full address
    return location;
  };
